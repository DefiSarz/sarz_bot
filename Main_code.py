...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('quality_'):
        ...
        if query.data == 'premium_yes':
            ...
            if pending:
                await query.edit_message_text("✅ Uploading your file...")
                ...
                await query.edit_message_text("❌ File expired. Please try downloading again.")
            ...
            await query.edit_message_text(
                f"ℹ️ *Telegram Premium Required*\n\n"
                f"📦 File size: {pending['file_size_mb']:.1f}MB\n"
                f"🔓 Regular limit: {REGULAR_USER_LIMIT_MB}MB\n\n"
                f"💡 *Options:*\n"
                f"1️⃣ Upgrade to Telegram Premium (4GB limit)\n"
                f"2️⃣ Select lower quality: /quality\n"
                f"3️⃣ Try audio-only: /audio\n"
                f"4️⃣ Download 720p or 480p\n\n"
    ...
    try:
        ...
        if file_size_mb > PREMIUM_USER_LIMIT_MB:
            ...
            await status_msg.edit_text(
                ...
                f"⚠️ Maximum limit: {PREMIUM_USER_LIMIT_MB}MB (Telegram Premium)\n\n"
                f"💡 *Solutions:*\n"
                f"• Select lower quality: /quality\n"
                f"• Try audio-only mode: /audio\n"
                f"• Download 720p or 480p instead",
                parse_mode=ParseMode.MARKDOWN
            )
            ...
            if user_has_premium:
                ...
                keyboard = [
                    [
                        InlineKeyboardButton("✅ I have Premium", callback_data=f"premium_yes|{url}|{quality}|{media_type}"),
                        InlineKeyboardButton("❌ No Premium", callback_data=f"premium_no")
                    ]
                ]
                reply_markup = InlineKeyboardMarkup(keyboard)
                ...
                await status_msg.edit_text(
                    ...
                    f"📦 File size: {file_size_mb:.1f}MB\n"
                    f"🔓 Regular limit: {REGULAR_USER_LIMIT_MB}MB\n"
                    f"⭐ Premium limit: {PREMIUM_USER_LIMIT_MB}MB\n\n"
